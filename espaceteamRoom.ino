...
void displayStartScreen() {
  while(currentScreen == START_SCREEN){
    ...
    tft.setTextSize(2);
    tft.setTextColor(TFT_GREEN, TFT_BLACK);
    tft.drawString("LEFT for room 1", 20, 50);
    tft.drawString("and RIGHT for room 2", 20, 80);
    tft.drawString("together", 20, 110);
...
void loop()
{
  ...
  if (redrawCmdRecvd || redrawProgress) {
    ...
    if (progress%20 == 0 && progress != 0 && progress < 100) {
      level = int(floor(progress/20));
      int newexpire = max(5,(level*4));
      expireLength = expireLength - newexpire;
      tft.fillScreen(TFT_BLUE);
      tft.setTextSize(1);
      tft.setTextColor(TFT_WHITE, TFT_BLUE);
      tft.drawString("LEVEL", 45, 20, 2);
      tft.drawString("UP!!!", 20, 80, 2);
      tft.drawString("Level: "+String(level), 18, 130, 2);
      delay(1500);
      progress = progress + 1;
      textSetup();
      // buttonSetup();
